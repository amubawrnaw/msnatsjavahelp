...
public class DriverCourse2S17
{
	public static void main(String[] args)
	{
		...
		for(i = 0; i < dlsuS.length; i++){
			...
			c.addStudent(dlsuS[i]);
			
		}
		
		//Calls addStudent constructor
		/*i = 0;
		while(i < dlsuS.length && c.addStudent(dlsuS[i]))
			i++;*/
		
		//Display contents of the class offered
		int k;
		System.out.println("\n\nCourses Offered: ");
		for(k = 0; k < classes; k++){
		...
		int CourseIndex;
		for(i = 0; i < dlsuS.length; i++){
			System.out.println("Student: " + dlsuS[i].getName());
